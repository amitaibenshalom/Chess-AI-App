...
public class AiHardActivity extends AppCompatActivity implements View.OnClickListener{
...
//        this.canClose = false;
    ...
    public void onBackPressed() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(AiHardActivity.this);
    ...
    public void gameEnded(int color) {
        ...
        backBtn2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                dialog.cancel();
                Intent intent = new Intent(AiHardActivity.this, MainActivity.class);
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivity(intent);
                finish();
            }
...
//        System.out.println(getMinimaxValue(2,1,board,false,-9999,9999));
        Point[] move = new Point[2];
//        ArrayList<Move> similarMoves = new ArrayList<Move>();
        int best = 99999;
        int promotion = -1;
        int piecesSize = board.getPlayers()[1].getPieces().size();
        if (piecesSize + board.getPlayers()[0].getPieces().size() <= 14)
            depth = 3;
        if (piecesSize + board.getPlayers()[0].getPieces().size() <= 7)
            depth = 4;
        if (piecesSize + board.getPlayers()[0].getPieces().size() <= 4)
            depth = 5;
